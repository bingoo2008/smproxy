 ...
 * 调试工具类。该类提供两个主要的静态方法：
 * <OL>
 * <LI>myAssert(boolean 条件);断言，用来确保本应成立的条件确实成立，防止 错误扩散。
 * <LI>dump(任何数据类型,递归深度);输出一个变量的内容，后一个参数可省， 缺省值为3。该方法可以输出任何数据类型的变量的内容，包括基本数据类型、数
 * 组和对象。对一些常用对象，如Vector等，有专门的输出格式。其他对象将输出其
 * 成员变量，包括私有变量和静态变量。输出过程会一直递归，递归层次由deep参数
 * 来控制，如果递归层次超过限制值，则直接输出toString()的内容，如果递归出现
 * 循环，则在发现重复引用处停止递归。输出信息除对象的内容外还包括输出时间、 调用dump函数的代码所在源文件，行号，若是对象还包括该对象的地址(注：虚拟
 * 机内部编址，非物理地址，可用来判断两个变量是否是同一个对象的引用)。
 * </OL>
...
public class Debug {
	...
	private static final void dumpServletRequest(String prefix, Object request) {
		try {
			...
			for (Enumeration e = (Enumeration) m1.invoke(request,
					new Object[] {}); e.hasMoreElements();) {
	...
	private static final void dump(String prefix, int depth,
		...
		while (e.hasMoreElements()) {
			dump(indent(prefix) + '[' + (i++) + "] ", depth, checkCircuit,
					e.nextElement());
		}
	...
	private static final void dump(String prefix, byte[] data, int offset,
		...
		if (offset < 0 || data.length < offset + length) {
			dump(prefix, "IndexOutOfBounds:data.length=" + data.length
		...
		for (int i = offset; i < end; i += 16) {
			byte[] row = new byte[] { 0x30, 0x30, 0x30, 0x30, 0x3A, 0x20, 0x30,
					...
					0x30, 0x20, 0x30, 0x30, 0x20, 0x30, 0x30, 0x20, 0x30, 0x30,
					0x20, 0x30, 0x30, 0x20, 0x30, 0x30, 0x20, 0x7C, 0x20, 0x2E,
	...
	private static final void dump(String prefix, int depth,
		...
		for (Iterator i = map.keySet().iterator(); i.hasNext();) {
			...
			if (value instanceof String) {
				...
				dump(indent(prefix) + key.toString() + " = ", depth,
						checkCircuit, map.get(key));
			}
		}
	...
	private static void dump(String prefix, int depth, Vector checkCircuit,
		...
		try {
			...
			if ((obj instanceof String) || (obj instanceof Number)
				...
				return;
			} else if (checkCircuit.contains(new Integer(System
				...
				return;
			} else if (Class.forName("javax.servlet.ServletRequest")
					.isInstance(obj)) {
	 ...
	 * 
	 * @param esc
	 *            调用堆栈中需要剔除的层次中的字符串，传入null表示无剔除层次。
	...
	private static final byte[] hexNumber = { (byte) '0', (byte) '1',
	...
	/**
	 * 将字符串长度变为固定长（后面补空格），若长度已经等于或超过期望长度则 补成期望长度的整数倍。
	 * 
	 * @param str
	 *            期望改变长度的字符串。
	 * @param len
...
/**
 * 断言失败时抛出的错误。只是简单从Error类继承。错误不是异常，系统不应该捕获的。 若系统抛出该错误说明有BUG。
 */
class AssertFailed extends Error {

	private static final long serialVersionUID = -4928822943272906498L;
}