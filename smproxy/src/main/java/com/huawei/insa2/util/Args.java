...
/**
 * 提供一个保存参数的容器，用来给那些参数多、可选参数多、参数易变的方法提供参数读取能力。 直接使用Map等类对于数据类型处理不是方便。本类提供常用数据类型的读取方法。用法示例：<br>
...
public class Args {
    ...
    /**
     * 锁定参数列表。锁定后该参数列表将变成只读的，任何对set()方法的调用都将抛出 UnsupportedOperationException异常。已经锁定的参数表再次调用本方法没有任何影响。
     * 
