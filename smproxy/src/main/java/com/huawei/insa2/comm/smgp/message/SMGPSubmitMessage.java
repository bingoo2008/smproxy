...
public class SMGPSubmitMessage extends SMGPMessage
{
    ...
    public SMGPSubmitMessage(int msgType, int needReport, int priority, String serviceId, String feeType, String feeCode, String fixedFee, 
            int msgFormat, Date validTime, Date atTime, String srcTermId, String chargeTermId, String destTermId[], byte[] msgContent,int tp_udhi, 
            String reserve)
...
//	        if(msgFormat == 15){
//	        	//GBK
//					contentBytes = msgContentTemp.getBytes("GBK");
//	        }else{
//	        	//UTF-8
//	        	contentBytes = msgContentTemp.getBytes("UTF-8");
//	        }
...
//		}
        ...
        int len = 126 + 21 * destTermId.length + msgContent.length+5;//5字节为  TLV TP_udhi值
...
//    	System.out.println("contentBytes.length -->"+msgContent.length);
//    	
//    	System.out.println(" contentBytes -- >"+bytesToHexString(msgContent));
//    	System.out.println("buf size -->"+buf.length);
        System.arraycopy(msgContent, 0, buf, loc + 1, msgContent.length);
//      System.out.println(" buf -- >"+bytesToHexString(buf));
        loc = loc + 1 + msgContent.length;
        if(reserve != null)
            System.arraycopy(reserve.getBytes(), 0, buf, loc, reserve.length());
        
//      System.out.println(" buf2 -- >"+bytesToHexString(buf));
        //拼接TLV TP_udhi值
        byte[] tp_udhi_TLV = new byte[5];
        tp_udhi_TLV[1] = 0x0002;
        tp_udhi_TLV[3] = 0x0001;
        if(tp_udhi == 1){
            tp_udhi_TLV[4] = 0x0001;	
        }
        loc = loc + 8;
        System.arraycopy(tp_udhi_TLV, 0, buf, loc, tp_udhi_TLV.length);
