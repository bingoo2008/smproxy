...
public abstract class PLayer {
    ...
    /**
     * 当父协议层收到消息时，通过getChildId()方法，根据消息中带的信息，找到相应的本协议层自 动机号，并自动回调其onReceive()的方法。本协议层的onReceive()方法再继续调度消息到更
     * 上一层协议自动机，直到达到最顶层协议自动机。 最顶层协议自动机应该覆盖onReceive()方法，将消息保存起来并提供取得消息的方法。如果
    ...
    /**
     * 取得该协议层的父协议层。如果当前协议层是最底层则返回null。 return 父协议层自动机对象。
     */
    ...
    /**
     * 从消息中获得调度本消息到子协议层自动机的自动机号。例如，如果连接层的上一层是会话层，则 连接层的getChildId()方法应该从消息中提取出会话Id来。如果该消息要求创建新的自动机则该
    ...
    /**
     * 关闭该自动机。缺省的实现是将自己从父协议层的子协议层列表中删除。最底层协议自动机必须 覆盖close()方法并做自己该做的事情，例如关闭Socket连接等。若不覆盖则会抛出异常。
    ...
    /**
     * 注册协议层自动机各种事件的监听器。如创建、删除、消息发送成功、消息发送失败、接收消息 消息。
     * 
     * @param l 待添加的事件监听器。
     */
    ...
    protected void fireEvent(PEvent e) {
        ...
        // 通知所有事件监听器
        for (PEventListener pl : listeners) {
            pl.handle(e);
        }
    }
}
