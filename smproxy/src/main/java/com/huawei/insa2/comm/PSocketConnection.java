...
public abstract class PSocketConnection extends PLayer {
    ...
    private static final Logger logger             = LoggerFactory.getLogger(PSocketConnection.class);
    ...
    /**
     * Socket读超时时间。读时超过该时间没收到数据认为连接中断，重新尝试连接。单位毫秒，0表示 永不超时。其值至少应大于通信对方发送心跳间隔的时间，若对方不发心跳则该值应设置为0。
     */
     ...
     * 默认到达重连次数阈值
     */
    protected static final int  RECONNECT_THESHOLD = 3;

    /**
     * 根据参数设置创建一个PSocket。由于本类的构造需要的参数较多，且很多是可选参数,因此采用 Args类来专门保存参数，而不是给构造方法定义一大堆参数。Args中保存的key数据类型应为字符
     * 串，value数据类型可以是字符串或Number等对象(例如："123"或new Integer(123)都可以)。 目前支持的参数有：
     * <ul>
     * <li>host ---- 主机地址。必选。可以是IP地址也可以是机器名或域名。
     * <li>port ---- 主机端口号。必选。有效范围0~65535。
     * <li>name ---- 为本连接起的名字。可选。缺省值是host与port的组合。
     * <li>reconnect-interval ---- 重连间隔时间，如果发现连接中断则等待这么长时间后尝试 重新建立连接。可选。单位：秒，缺省60秒。
     * <li>heartbeat-interval ---- 心跳间隔时间。可选。单位：秒，缺省为0表示不发心跳消息。
     * <li>read-timeout ---- 读数据超时时间。可选。单位：秒，缺省为0表示永不超时。该值至少 要大于通信对方发送心跳的间隔时间。
    ...
    protected void init(Args args) {
        ...
        class ReceiveThread extends WatchThread {
            ...
            private final Logger logger = LoggerFactory.getLogger(PSocketConnection.class);
                    ...
                    if (isRunningOk()) {
                        ...
                        if (!error.equals(NOT_INIT)) { // 如果不是第一次建立连接
                            try {
                                ...
                                logger.error("sleep interval error", ex);
                            }
                        }

                        if (error == CONNECTING || error == RECONNECTING) {
                            // 如果正在连接。则直接结束。防止重复重连
                            logger.warn("connecting..." + error);
                            return;
                        }

                        logger.info("connecting ---> " + error);
                        // LogUtil.info(cts.toISO("系统重新与网关连接！........................"),PSocketConnection.class);
                        connect();
                    }
                } catch (Exception ex) {
                    logger.error("ReceiveThread task error ", ex);
            ...
            public void task() {
                ...
                if (error == null && out != null) {
                    try {
                        ...
                        logger.error("heartbeat task error ", ex);
    ...
    public void setAttributes(Args args) {
        ...
        // 本地地址（域名或IP），目前这两个参数主要为了短连接（联通SGIP协议）需要在本地加入监听
        ...
        // 判断是否需要重连
        reconnectIfDifferentArgs(args);
    }

    /**
     * TODO 重连（主机地址或端口号改变则需要重连）
     * 
     * @param args
     */
    private void reconnectIfDifferentArgs(Args args) {
        if (args.isEmpty()) {
            return;
        }

        if (isRunningOk() && isSameArgsConnect(args)) {
            // 令接收线程立即重连
     ...
     * @return
     */
    private boolean isSameArgsConnect(Args args) {
        return isStrSame(host, args.get("host")) && isStrSame(port + "", args.get("port"));
    }
     ...
     * @return
     */
    private static boolean isStrSame(String str1, String str2) {
    ...
    protected synchronized void connect() {
        ...
        if (socket != null) { // 如果socket对象存在，在连接前先把它关闭。
            try {
                ...
                logger.error("connect socket.close error", ex);
        ...
        try {
            ...
            logger.error("socket connect error ", ex);
    ...
    /**
     * 执行定期心跳。缺省什么也不做，须心跳的协议子类须重载该方法。 如果协议中心跳是有响应的，则等待响应。可在连续失败计数达到累计值时抛出IOException异常 则连接将会自动重新建立。
     */
