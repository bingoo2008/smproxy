...
public class SGIPConnection extends SGIPSocketConnection {
	...
	int degree = 0;// 计数器,计算连续发出的心跳消息没有响应的个数
	int hbnoResponseOut = 3;// 连续发出心跳消息没有响应达到该数，需要重连
	String source_addr = null;
	int version;// 双方协商的版本号
	String shared_secret;// 事先商定的值，用于生成SP认证码
	...
	public synchronized void attach(Args args, Socket socket) {
		if (asClient) {
			throw new UnsupportedOperationException(
					"Client socket can not accept connection");
		} else {
			init(args, socket);
	...
	public int getChildId(PMessage message) {
		...
		if (!asClient)
			;
		if (mes.getCommandId() == 4 || mes.getCommandId() == 5
				|| mes.getCommandId() == 17 || mes.getCommandId() == 1
				|| mes.getCommandId() == 2)
			return -1;
		else
			return sequenceId;
...
//			SGIPMessage sgipmessage = t.getResponse();
		...
		if (!asClient && connmap != null)
			connmap.remove(new String(String.valueOf(ipaddr)
					+ String.valueOf(port)));
	}
